...
'''
def get_prompts(prompt,temperature=0.7, max_tokens=1670,number_of_results=1):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Cmdline arguments to run the gpt prompt generator')
    parser.add_argument("--prompt",type=str,help="prompt for generating recipe")
    parser.add_argument("--temperature",type=str,help="temperature between 0.7 and 0.9")
    parser.add_argument("--max_tokens",type=str,help="max input tokens",default=1670)

    args = parser.parse_args()
    prompts = get_prompts(args.prompt,args.temperature, args.max_tokens,number_of_results=1)
